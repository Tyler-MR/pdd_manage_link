...
def list_xlsx_files():
    """只列出 6月、7月目录下的直接 xlsx 文件，不递归扫描子目录。"""
    ...
    files = []
    for month, month_name in ((6, "6月"), (7, "7月")):
...
def _creation_date_expression(alias="cp"):
    """把 pdd_profile / pdd_link_info 中的多种创建时间格式统一成 DATE。"""
    return (
        f"COALESCE("
...
def _creation_link_ids(conn, creation_days=None, creation_start=None, creation_end=None):
    ...
    if end:
        conditions.append(f"DATE({date_expr}) <= %s")
    cur = conn.cursor()
    cur.execute(f"SELECT DISTINCT cp.`链接ID` FROM pdd_link_info cp WHERE {' AND '.join(conditions)}", [value for value in (start, end) if value])
...
def _append_creation_filter(where, params, creation_days=None, creation_start=None, creation_end=None, link_column="`链接id`", conn=None):
    """按链接创建日期过滤，日期来源为 pdd_profile，范围含首尾日期。"""
    start, end = _creation_window(creation_days, creation_start, creation_end)
    if not start and not end:
        return
    ...
    if end:
        conditions.append(f"DATE({date_expr}) <= %s")
        params.append(end)
    where.append(f"{link_column} IN (SELECT cp.`链接id` FROM pdd_profile cp WHERE {' AND '.join(conditions)})")


def _creation_select_expression(outer_table=TABLE_NAME):
    return f"(SELECT MAX(cp.`创建时间`) FROM pdd_link_info cp WHERE cp.`链接ID` = {outer_table}.`链接id`) AS `链接创建时间`"
    ...
    for item in filters:
        ...
        if field == '链接创建时间':
            ...
            if op not in ('gte', 'lte', 'between') and v1:
                ...
                params.append(v1)
            if len(conditions) > 1:
                where.append(f"{TABLE_NAME}.`链接id` IN (SELECT cp.`链接ID` FROM pdd_link_info cp WHERE {' AND '.join(conditions)})")
            continue
...
def _get_aggregated_link_person_page(conn, where, params, deferred_numeric_filters, page, size):
    ...
    query = (
        ...
        f"FROM pdd_link_info GROUP BY `链接ID`) link_created "
